...
def compute_ece(probs: np.ndarray, true_labels: np.ndarray, n_bins: int = 15) -> float:
    """
    Compute Expected Calibration Error (ECE) using confidence binning.
    ...
    n_bins : int, default=15
        Number of bins for grouping predictions by confidence.
    ...
    for bin_lower, bin_upper in zip(bin_lowers, bin_uppers):
        ...
        prop_in_bin = np.mean(in_bin) #counts booleans
...
def compute_ece_adaptive(probs: np.ndarray, true_labels: np.ndarray,
                         min_samples_per_bin: int = 50, max_bins: int = 15) -> float:
    """
    Compute Expected Calibration Error (ECE) using adaptive binning.

    Unlike standard ECE, this version uses adaptive binning to ensure each bin
    has a minimum number of samples for more robust statistical estimates.
    The number of bins is automatically reduced if the dataset is too small.
    ...
    min_samples_per_bin : int, default=50
        Minimum number of samples required per bin for reliable statistics.
    max_bins : int, default=15
        Maximum number of bins to use (actual bins may be fewer based on dataset size).

    Returns
    -------
    ece : float
        Expected Calibration Error with adaptive binning (lower is better, 0 is perfectly calibrated).

    Notes
    -----
    This implementation addresses the issue of noisy estimates in bins with very few samples
    by dynamically adjusting the number of bins based on the dataset size.
    """
    probs = np.asarray(probs)
    true_labels = np.asarray(true_labels)
    ...
    n_bins = min(max_bins, max(5, n_samples // min_samples_per_bin))
...
def compute_brier_score(probs: np.ndarray, true_labels: np.ndarray) -> float:
    """
    Compute Brier score for multiclass classification.

    The Brier score is a proper scoring rule that measures the mean squared
    difference between predicted probabilities and the actual outcome (one-hot).
    ...
    # Brier score: mean squared error between predicted probs and one-hot labels
    brier = np.mean(np.sum((probs - one_hot) ** 2, axis=1))

    return brier


def compute_conformal_aps(probs: np.ndarray, true_labels: np.ndarray,
                          target_coverage: float = 0.95, cal_ratio: float = 0.2,
                          seed: int = 42) -> tuple[float, float]:
    """
    Compute conformal prediction metrics using Adaptive Prediction Sets (APS).

    Splits the test set into calibration and evaluation portions to compute
    valid coverage guarantees and measure prediction set efficiency.
    ...
    cal_ratio : float, default=0.2
        Fraction of data to use for calibration (rest used for evaluation).
    seed : int, default=42
        Random seed for reproducible calibration/evaluation split.

    Returns
    -------
    coverage : float
        Empirical coverage on evaluation set (fraction where true label is in prediction set).
    avg_set_size : float
        Average number of classes in prediction sets on evaluation set.

    Notes
    -----
    Uses the Adaptive Prediction Sets (APS) method which builds prediction sets by
    including classes in descending probability order until cumulative probability
    exceeds a threshold determined from the calibration set.

    The non-conformity score measures how much probability mass was placed on
    incorrect classes before the true label (higher = more uncertain/incorrect).
    The quantile of these scores determines the prediction set threshold.
    """
    probs = np.asarray(probs)
    true_labels = np.asarray(true_labels)

    n_samples = len(true_labels)
    n_cal = int(n_samples * cal_ratio)

    # Split into calibration and evaluation sets with fixed seed
    np.random.seed(seed)
    ...
    for i in range(n_cal):
        ...
        sorted_indices = np.argsort(cal_probs[i])[::-1]
        sorted_probs = cal_probs[i][sorted_indices]

        # Find position of true label in sorted order
        true_label_pos = np.where(sorted_indices == cal_labels[i])[0][0]

        # Non-conformity score: cumulative probability up to (but not including) true label
        # Higher score = more non-conforming = model placed more probability on wrong classes
        non_conformity_score = np.sum(sorted_probs[:true_label_pos])
    ...
    # Formula ensures (1-alpha) coverage in expectation
    alpha = 1 - target_coverage
    q_level_raw = np.ceil((n_cal + 1) * (1 - alpha)) / n_cal
    q_level = min(q_level_raw, 1.0)  # Cap at 1.0
    quantile_threshold = np.quantile(non_conformity_scores, q_level)
    # === Evaluation Phase ===
    ...
    for i in range(len(eval_indices)):
        # Sort classes by probability (descending)
        sorted_indices = np.argsort(eval_probs[i])[::-1]
        sorted_probs = eval_probs[i][sorted_indices]

        # Build prediction set: include classes until cumulative prob >= threshold
        cumsum = np.cumsum(sorted_probs)
        set_size = np.searchsorted(cumsum, quantile_threshold, side='left') + 1
        set_size = min(set_size, len(sorted_indices))  # Cap at number of classes

        prediction_set = sorted_indices[:set_size]

        # Check coverage: is true label in prediction set?
        coverage_i = eval_labels[i] in prediction_set
